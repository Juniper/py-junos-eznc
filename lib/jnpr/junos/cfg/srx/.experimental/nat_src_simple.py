...
class NatSourceSimple(TemplateResource):
    ...
    def _r_xpath_names(self, name):
        ...
        if isinstance(name, dict):
            ...
            try:
                ...
                if not t_names.get('pool_name'):
                    t_names['pool_name'] = name['ruleset_name']
    ...
    def _r_template_write_vars(self):
        ...
        # these markers are specific to the actual template (see file for
        # details)

        if 'pool_from_addr' in self.should or 'pool_to_addr' in self.should:
            t_vars['_pool_'] = True

        if 'zone_from' in self.should or 'zone_to' in self.should:
            t_vars['_rule_set_'] = True

        if 'match_src_addr' in self.should or 'match_dst_addr' in self.should:
            t_vars['_rule_set_'] = True
            t_vars['_rule_'] = True
