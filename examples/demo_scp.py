...
def approach_3(src_path, dst_path='/var/tmp'):
    ...
    def scp_progress(_path, _total, _xfrd):
        ...
        if not hasattr(scp_progress, 'by10pct'):
            scp_progress.by10pct = 0
        ...
        if 0 == (pct % 10) and pct != scp_progress.by10pct:
            ...
            print "%s: %s / %s (%s%%)" % (_path, _xfrd, _total,
                                          str(pct))
...
# example output:
# >>> approach_3('/usr/local/junos/packages/junos-vsrx-12.1X44-D20.3.tgz')
