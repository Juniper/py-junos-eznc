...
class PhyPortSwitch( PhyPortBase ):
  ...
  def _xml_to_py(self, has_xml, has_py ):
    ...
    has_py['loopback'] = bool(ethopts.find('loopback') is not None)
    ...
    if 'duplex' in has_py: PhyPortBase._set_invert(has_py, 'duplex', self.PORT_DUPLEX)
  ...
  def _xml_hook_build_change_begin(self, xml):
    if any([ this in self.should for this in ['speed','duplex','loopback']]):
