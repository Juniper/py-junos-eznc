...
class UAC(object):
    ...
    def user(self, name):
        ...
        def _match(x):
            return name == x['name']  # re.search(name, x['username'])
        return filter(_match, self.users)[0]

    def _flows_by_srcprefix(self, ip_prefix):
        return self.dev.rpc.get_flow_session_information(
            source_prefix=ip_prefix)
    ...
    def user_byteusage(self, user_name):
        ...
        sflows = sorted(
            flows.xpath('.//flow-information'),
            key=lambda x: x.find('direction').text.strip())
        sums = {}
        for fdir, gdata in groupby(sflows,
                                   lambda x: x.find('direction').text.strip()):
